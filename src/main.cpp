...
const string strMessageMagic = "EventToken Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if(pindexBest->nHeight == 5)
        {
            nSubsidy = 400000 * COIN; // 400k coins for ico
        }
	else if (pindexBest->nHeight > 6 && pindexBest->nHeight < 100 )
    {
        nSubsidy = 0 * COIN; 
    }
  	else if (pindexBest->nHeight > 99 && pindexBest->nHeight < 10101)
    {
        nSubsidy = 200 * COIN; 
    }
	else if (pindexBest->nHeight > 10100 && pindexBest->nHeight < 20001 )
    {
        nSubsidy = 400 * COIN; 
    }
	else if (pindexBest->nHeight > 20000 && pindexBest->nHeight < 31001 )
    {
        nSubsidy = 200 * COIN; 
    }
		else if (pindexBest->nHeight > 31000 && pindexBest->nHeight < 40000 )
    {
        nSubsidy = 100 * COIN; 
    }
    else if (pindexBest->nHeight > 40000 && pindexBest->nHeight < 50001 )
    {
        nSubsidy = 50 * COIN; 
    }
...
const int POS_BLOCK_BEFORE_FIX = 55000;
const int POS_BLOCK_REWARD_FIX = 54999;
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindex, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindex->nHeight < POS_BLOCK_BEFORE_FIX)
	{

		int64_t nSubsidy = nCoinAge * nRewardCoinYear / 365 / COIN;
         return nSubsidy + nFees;

    } else if (pindex->nHeight > POS_BLOCK_REWARD_FIX) {
...
static const int64_t nTargetTimespan = 21 * 60;  // 21 mins
...
// EventToken: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EventToken", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "November EventToken 2014";
        CTransaction txNew;
        txNew.nTime = 1417719265;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	printf("%s\n", hash.ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0x6f235592ab7e2968655d695345a90650c54f731e6c3f73d87e70e446498f5af3"));
...
unsigned char pchMessageStart[4] = { 0xcc, 0x3b, 0x41, 0xff };
